...
function get_first_isa(agent::HSAAgent)
    isa = Ref{HSA.ISA}()
    func = @cfunction(agent_iterate_isas_cb, HSA.Status, (HSA.ISA, Ptr{HSA.ISA}))
...
function Base.show(io::IO, agent::HSAAgent)
    print(
        io,
        "HSAAgent($(agent.agent)): Name=$(get_name(agent)), Type=$(device_type(agent))",
    )
end
...
function get_agents()
    ...
    GC.@preserve agents begin
        func = @cfunction(iterate_agents_cb, HSA.Status, (HSA.Agent, Ref{Vector{HSAAgent}}))
        HSA.iterate_agents(func, agents) |> check
        _agents = agents[]
    end
    return _agents
end
get_agents(kind::Symbol) = filter(agent -> device_type(agent) == kind, get_agents())
...
function get_region(agent::HSAAgent, kind::Symbol)
    ...
    if kind == :finegrained
        func = @cfunction(
            get_fine_grained_memory_region_cb,
            HSA.Status,
            (HSA.Region, Ptr{HSA.Region})
        )
    elseif kind == :kernarg
        func = @cfunction(
            get_kernarg_memory_region_cb,
            HSA.Status,
            (HSA.Region, Ptr{HSA.Region})
        )
    else
