...
const HSA_MUTABLE_TYPES = Union{Region}

# Convenience constructors
const HSA_ZEROABLE_TYPES = Union{Region}

newref!(::Type{Ref{T}}, value) where T<:HSA_ZEROABLE_TYPES = Ref{T}(T(value))

# Memory
memset!(ref::Ref{T}, value::T) where T<:Integer =
    _memset!(ref, value, T)
...
function getinfo(agent::Agent, attribute::AgentInfo, value::String)
    ccall((:hsa_agent_get_info, "libhsa-runtime64"), Status, (Agent, AgentInfo, Cstring), agent, attribute, value)
end
function getinfo(agent::Agent, attribute::AgentInfo, value::Ref{T}) where T
    ccall((:hsa_agent_get_info, "libhsa-runtime64"), Status, (Agent, AgentInfo, Ref{T}), agent, attribute, value)
end

function getinfo(isa::ISA, attribute::ISAInfo, value::Ref{T}) where T
    ccall((:hsa_isa_get_info_alt, "libhsa-runtime64"), Status, (ISA, ISAInfo, Ref{T}), isa, attribute, value)
end
function getinfo(isa::ISA, attribute::ISAInfo, value::String)
    ccall((:hsa_isa_get_info_alt, "libhsa-runtime64"), Status, (ISA, ISAInfo, Cstring), isa, attribute, value)
end

function getinfo(exsym::ExecutableSymbol, attribute::ExecutableSymbolInfo, value::String)
    ccall((:hsa_executable_symbol_get_info, "libhsa-runtime64"), Status, (ExecutableSymbol, ExecutableSymbolInfo, Cstring), exsym, attribute, value)
end

function memory_allocate(region::Region, size::Integer, ref::Ref{Ptr{T}}) where T
    ccall((:hsa_memory_allocate, "libhsa-runtime64"), Status, (Region, Csize_t, Ref{Ptr{T}}), region, size, ref)
end
